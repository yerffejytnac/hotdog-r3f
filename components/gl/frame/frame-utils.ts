import type { Vector3D } from "@/types/frame"
...
export const calculateRailPositions = (width: number, height: number, profileWidth: number) => {
  const halfW = width / 2
...
export const calculateRailScales = (
  ...
  return {
    horizontal: [horizLen / originalSize.x, profileWidth / originalSize.y, profileDepth / originalSize.z] as [
      number,
      number,
      number,
    ],
    vertical: [vertLen / originalSize.x, profileWidth / originalSize.y, profileDepth / originalSize.z] as [
      number,
      number,
      number,
    ],
